...
Ranger is the implementation of soma `docs/work-graph.md` §5 Phase 2 — the headless tick under a machine-account PAT, the Discord claim announcement, and the GitHub Actions close auditor. That spec is already decided; ranger builds it rather than rivaling it, and extends it with the node-kind routing and SOP integration the spec stops short of. **One per-map deviation (node #7, closed):** the §5 Phase 2 60s claim-veto window is replaced by a blocking-vs-non-blocking model for ranger walks — claims and gated merges proceed automatically, blocking items wait indefinitely for the principal.
...
launchd (interval) ──► ranger tick ──┬─ guards (run-lock, dead-man, identity, cortex health)
                                     ...
                                     ├─ claim     (announce → soma graph claim — auto, no veto)
...
| `soma graph release` | SHIPPED | Identity-bound self-release verb (promotes the claim-race loser's existing self-removal) — merged upstream 2026-08-16 (soma#628, spec #627). Ranger workers self-release abandoned claims; no more escalate-only. |
| Machine-account PAT, approver token, Discord bot, probe-registry entries, launchd plist | PROVISIONING | §9 (machine-account write PAT = classic `repo`-scoped, node #11 — fine-grained blocked for outside collaborators + personal repos, 2026-08-16). |
...
1. **ivy-agent** (the machine account — `jcfischer+ivy@gmail.com`, created 2026-08-02; the design's earlier `Ranger-bot` placeholder is an existing standalone account treated as residue). Claims nodes, authors branches and PRs, closes nodes. `GH_TOKEN` = its classic `repo`-scoped PAT for ranger's entire process tree. **Credential note (2026-08-16, node #11):** GitHub blocks fine-grained PATs for outside collaborators (ivy-agent is not a `the-metafactory` org member — GitHub Team, 0 free seats, see node #16) and fine-grained PATs can never be scoped to another account's personal repos (`jcfischer/*`); the classic `repo`-scoped PAT is the fallback the docs specify for outside collaborators, covers both org and personal walked repos with one token, and keeps the substantive invariant — no autonomous graph-mutation under the principal's credential. Ranger **refuses any graph-mutating tick** under a resolved identity equal to the principal's login — "no autonomous graph-mutation under the principal's credentials" made mechanical (node #8: reads are outside the doctrine's teeth). Read-only components (currently scout) are exempt via the read-only token gate — explicit read-only credential, never `gh` keyring fallback (the keyring token is write-capable), abort on write scopes, fixed read-only verb surface (`audit`/`frontier`/`node`); they may run under any identity, principal included. The token gate is identity-agnostic: it ports to the machine account unchanged when it provisions. Note: fine-grained PATs are org-scoped; covering `the-metafactory/*` and `jcfischer/*` needs two PATs (per-repo mapping in `ranger.yaml`).
...
**The self-amplification ban (node #9, settled):** ranger-filed nodes (§6) are `propose` minimum — ranger never grants `auto` to its own filed work. Only a human grants `auto`. Two settled amendments: (a) ranger-filed *research* nodes may be filed `auto` — research is the one kind with autonomous precedent and is parallel-safe by doctrine; (b) the digest card for a ranger-filed `propose` node carries a one-tap "make auto" for the principal — the tap is the human granting `auto`, so the ban holds. This kills the loop the critique found (machine mints auto work → claims it next tick → mints more) at the cost of one tap per deferred trivial item.
...
- Env: `GH_TOKEN`=ivy-agent PAT, `SOMA_GRAPH_REPO=<owner/repo>`, `SAGE_STACK=default`, `PILOT_PRINCIPAL=jc`. No approver token, no keychain export, no access to `~/.soma/policy/`.
...
**Channel:** Discord, one channel **per run** (configured in that map's `ranger.yaml` — decided individually per walk, not a global `#ranger`), one thread per map. Discord is notification and ratification surface; **GitHub is where answers bind**. Announce mechanics fail closed: no confirmed message id → no claim, no merge, no action.
...
| `approve` node — any | Notification only. No proposal, no recommendation-as-default, no action on silence. | Interactive session or explicit operator verb. No response-window exists anywhere: automatic work proceeds on its own, blocking work waits for the principal. |
| Operational stalls | Parked PRs (review cap), close-blocked probes, budget exhaustion, registry needs, audit findings, stale foreign claims, DORMANT lane, dead-man trips | Operator verbs: `ranger resume-node`, `resume-run`, `clear-veto`; or the principal's hands (registry paste, manual unassign of foreign claims — `soma graph release` now covers self-release). |
...
**Blocking vs non-blocking (node #7, closed).** Window-based vetoes are replaced by a categorical split. **Automatic (proceed without waiting for the principal):** claims (announce, then proceed immediately — **no veto on claims**) and merges (through the six-check gate under the approver identity — **no merge-veto window**; until the approver identity exists, merges escalate as blocking one-tap items per node #6). Announce-fail-closed still gates both (API-level, not a response wait). **Blocking (wait indefinitely until the principal responds — no short windows, no silence-proceeds):** proposal ratifications, grilling/approve decisions, operational stalls, provisioning requests. Cards persist; only the principal's response or an operator verb resolves them. This amends the work-graph.md §5 Phase 2 60s-claim-veto default for ranger walks (principal's per-map decision).
...
| Stale ranger claim, no recoverable worker | Sweep | Self-release via `soma graph release <id>` (the claim-race loser's DELETE-self, now a verb). Foreign claims stay escalate-only — ranger never unassigns another identity. |
...
| Veto / refusal | 👎 on a proposal comment, or a principal response to a blocking card | Sticky (node comment + journal), never auto-retried; `ranger clear-veto` only. Claims are never vetoed (node #7). |
...
4. ~~Escalation surface~~ — **decided (#7, closed):** channel is per-run config (ranger.yaml), not global; window-based vetoes replaced by blocking-vs-non-blocking (claims + gated merges automatic, no claim veto, no merge-veto window, announce-fail-closed still gates both; blocking items wait indefinitely for the principal); ratifier pinned to the principal; daily digest with aged cards + audit + budget; age-banded re-surface (3+ louder, 7+ @-mention).
5. ~~Scout credential ruling~~ — **decided (#8, closed):** reads are outside the doctrine's teeth — the constraint is amended to "no autonomous graph-mutation under the principal's credentials, ever". Read-only components may run under any identity, principal included, token-gated read-only (explicit read-only credential, no keyring fallback, fixed read-only verb surface); graph-mutation stays machine-account-only. Scout (#12) ships under the principal's two read-only fine-grained PATs; the digest still waits for the machine account (ivy-agent).
6. ~~Walk-mode opt-in and charting guidance~~ — **decided (#9, closed):** `ranger.yaml` registers walked maps with `walk: observe | research-only | full` — machine-readable build authority, never inferred from map-Notes prose (`full` gates the implement lane). Ranger-filed nodes are `propose` minimum — research may be filed `auto`, and the digest offers a one-tap "make auto" (the tap is the human's grant). Command probes run in the canonical checkout (`~/work/ranger-repos/<repo>`, pre-registered cwd); ranger-filed auto probes use ungated types only (git-ref-exists / artifact-exists atRef / git-merged-into). Orienteer's ChartTheMap gains a "charting for ranger" addendum — UPSTREAM to the skill, manual cherry-pick discipline.
7. ~~Upstream verbs~~ — **decided (#10, closed):** `soma graph release` shipped upstream (soma#628, spec #627); `close --json` noted as optional follow-up in the spec.
